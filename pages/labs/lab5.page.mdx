...
export const documentProps = { title: 'Lab 5' }
...
In this lab, you will learn about the data structures used to represent 3d scenes. You will write code that turns an easy-to-build representation of scenes, ```SceneData```, into ```RenderData```, which is much easier to render an image from. You will learn how lights, cameras and primitives are represented, as well as how to read **transformation graphs** so that every object has the correct final size and position in the scene. 

:::warning
You will need the code you’ve written in this lab for the upcoming assignments, including the [Ray](/projects/ray) and [Realtime](/projects/realtime) projects!
:::

### Scene Data vrs. Render Data
...
See the drawing below. Your goal is to tranform ```SceneData``` to ```RenderData```. Don't worry if you don't understand all the elements in the image, they'll be covered in this lab!

<ImageFigure
  images={[{ src: '/labs/lab5/SceneVrsRender.png', alt: 'TODO' }]}
  figNumber={1}
  figureCaption={'Transforming SceneData into RenderData'}
/>

<details>
  <summary>Advanced: Why do we use an Array to render primitives?</summary>
...
_Parts taken from [this article](https://docs.unity3d.com/Manual/Lighting.html) where you can read more about types of light used in 3-d scenes._
...
Notice that unlike the lights or camera structs, there aren't any fields in ```CS123ScenePrimitive``` that describe position or orientation. This is because it is more practical to use **scene graphs**  to manage this, especially when we have a lot of primitives!

## Scene Graphs

A **scene graph** is a directed acyclical graph that holds all the primitives as well as the transformations applied to them. For this reason they can also be called **transformation graphs**.
...
<ImageFigure
  images={[{ src: '/labs/lab5/Parsing_Lab_Simple_Graph-2.png', alt: 'TODO' }]}
  figNumber={2}
  figureCaption={'Transformation Graph with 5 Primitives'}
/>

As mentioned in **3.2.**, all nodes are ```CS123SceneNode```s and can be thought of as groupings of objects. All leaf nodes contain only ```primitives```. All non-leaf nodes contain ```transformations``` (denoted ```T1```, ```T2```,```R1``` etc.). The transformation(s) of non-leaf node are applied to the ```children``` nodes.

:::task
Write the final transformation matrix for the Cube, Cone and Cylinder.
    - A final transformation matrix is a matrix that transforms a primitive to its final position and orientation in the scene.
    - Your answer should be a product of matrices ```T1```, ```R2```, etc.
    - Keep in mind that order matters when multiplying matricies!
:::

#### A More Complex Transformation Graph
Now, consider the graph below:
 
<ImageFigure
  images={[{ src: '/labs/lab5/Parsing_Lab_City_Graph_v6.png', alt: 'TODO' }]}
  figNumber={3}
  figureCaption={'Transformation Graph with 8 Primitives'}
/>
...
:::task
Write a matrix ```M1``` that is all the transformations applied only to Cube 2. <ul> <li> Your matrix ```M1``` should be in the form of a product of scaling, rotation and translation matrices. </li>  <li> You can write each scaling, rotation and translation matrices in the form ```S(x, y, z)```, ```R(x, y, z, angle)``` and ```T(x, y, z)```.</li> </ul>
:::

:::task
 Write the final transformation matrix for cubes 1, 3 and 8. <ul> <li> This should again be a product of scaling, rotation and translation matrices in the form ```S(x, y, z)```, ```R(x, y, z, angle)``` and ```T(x, y, z)```.</li> </ul>
:::
...
Now that you know all about global data, lights, cameras, primitives and transformations, you can code them into .

You will implement a depth first search to generate the final transformation matrices for each primitive. To do this, you will be filling in out a struct called CS123SceneMetaData in `CS123SceneLoader::load`, based off of data found in the scene provided by a SceneData struct (SceneData.h).


### Implementing CS123SceneLoader::load()
...
## (Optional) Creating your own 3-d scenes
